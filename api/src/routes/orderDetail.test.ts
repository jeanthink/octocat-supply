...
describe('OrderDetail API', () => {
  beforeEach(async () => {
    ...
    // Seed full chain: headquarters -> branch -> order; supplier -> product
  ...
  it('should create a new order detail', async () => {
    ...
    expect(response.body).toMatchObject({ orderId: 1, productId: 1, quantity: 10 });
  ...
  it('should get an order detail by ID', async () => {
    const createResponse = await request(app).post('/order-details').send({
      orderId: 1,
      productId: 1,
      quantity: 5,
      unitPrice: 9.99,
  ...
  it('should update an order detail by ID', async () => {
    ...
    const response = await request(app).put(`/order-details/${orderDetailId}`).send({
      orderId: 1,
      productId: 1,
      quantity: 8,
      unitPrice: 9.99,
    ...
    expect(response.body.notes).toBe('Updated quantity');
  });
  ...
  it('should return 404 for non-existing order detail on PUT', async () => {
  ...
  it('should return 404 for non-existing order detail on DELETE', async () => {
